function fetch_tabulatordata_and_build_table(map_cfg, map, table_cfg, markers) {
  ...
  if (map_cfg.json_url.length !== 0) {
    fetch(map_cfg.json_url)
      ...
      .then(function (tabulator_data) {
        ...
        populateMapFromTable(table, map, map_cfg.on_row_click_zoom, markers);
      })
      .catch(function (err) {
        console.log(err);
      });
...
function init_map_from_rows(rows, markers) {
  ...
  rows.forEach((row) => {
    ...
    let coordinate_key = get_coordinate_key_from_row_data(row_data);
    var marker = L.marker(new L.LatLng(row_data.lat, row_data.lng), {
      title: get_tooltip_label(row_data),
    });
...
function populateMapFromTable(table, map, on_row_click_zoom, markers) {
  table.on("tableBuilt", function () {
    console.log("built table");
    let all_rows = this.getRows();
    var existing_markers_by_coordinates = init_map_from_rows(all_rows, markers);
...
function build_map_and_table(
  ...
  if (tms_cfg !== null) {
    // this is for the page gui / switch for toggling overlays
    let tms_layer = L.tileLayer(
      "https://emt-project.github.io/mapserver/map_01/{z}/{x}/{y}.png",
      tms_cfg.layerOptions,
    );
    tms_layer.addTo(map);
