    ...
    <xsl:template name="blockquote">
        ...
        <div>
            ...
            <blockquote class="blockquote">
                <p>
                    <xsl:value-of select="$project_title"/>is published by Susan Schreibman and the Austrian Centre for Digital Humanities, Vienna, 2026 (<a href="{$fullUrl}"><xsl:value-of select="$fullUrl"/></a>)
                </p>
            </blockquote>
        </div>
    </xsl:template>
